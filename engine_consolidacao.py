...
1. Mecanismo de Expansão Horizontal: Cria dinamicamente novas colunas no Md quando há dias de Julho que não existem
2. Espelhamento Vertical com Sangria: Transporta nativamente o valor da Sangria (Linha 42)
3. Preservação de Fórmulas: Injeta fórmulas literais nas linhas 14 e 40 do Md
"""
...
logger = logging.getLogger("EngineConsolidacaoV2")

# AJUSTE PARA MOCK_BOX DURANTE DESENVOLVIMENTO
PASTA_LAB = os.path.expanduser("~/work_out")
PASTA_VENDAS = os.path.join(PASTA_LAB, "mock_box", "Padroeira_vendas")
PASTA_RESTAURANTE_ANO = os.path.join(PASTA_LAB, "mock_box", "Restaurante", "A2026")

class EngineConsolidacao:
    """
    Engine de Consolidação para a Arquitetura de Reconciliação V2
    """
    ...
    def normalizar_data(self, valor) -> datetime.date:
    ...
    def executar_motor_unificado(self) -> dict:
        """
        ...
        try:
            ...
            if not os.path.exists(caminho_caixa2) or not os.path.exists(caminho_mensal):
                error_msg = "Arquivos base não encontrados no mock_box"
            ...
            for col in range(2, ws_me_lei.max_column + 1):
                ...
                v_total = ws_me_lei.cell(row=37, column=col).value  # Linha 37: Faturamento (âncora)

                if v_data:
                    dt_norm = self.normalizar_data(v_data)
                    if dt_norm:
                        datas_no_mensal[dt_norm] = col
                        # Se o dia existe, mas o faturamento está vazio/zerado, agenda para receber carga
            ...
            # Ajuste para a estrutura real do Caixa 2 (coluna 1: Data, coluna 2: Dia, etc.)
            for col in range(1, ws_cx.max_column + 1):
                v_data_cx = ws_cx.cell(row=1, column=col).value  # Linha 1: Data
                if v_data_cx and col > 1:  # Ignora a primeira coluna (cabeçalho)
            ...
            if not colunas_pendentes_de_carga:
                ...
                # Executa o espelhamento vertical
                for dia in dias_para_processar:
                    ...
                    # Linha 6 até o final (incluindo a linha 42 - Sangria)
                    for linha in range(6, ws_cx.max_row + 1):
                        # Preserva fórmulas vitais nas linhas 14 e 40
                        if linha == 14:
                        ...
                        # Injeta valor bruto (incluindo a linha 42 - Sangria)
                ...
                logger.info(f"[+] Espelhamento concluído! {mudancas} células tratadas (incluindo Sangria na linha 42).")
                self.status["injection"] = "success"
            ...
            try:
                ...
                logger.info(f"\n[SUCESSO] Arquivo Movto_diario.{self.aamm}.xlsx finalizado e salvo no mock_box!")
...
if __name__ == "__main__":
    logger.info("[*] =================================================================")
    logger.info("[*] CÓRTEX PADROEIRA - MOTOR UNIFICADO (DIÁRIO MENSAL) - V2")
    logger.info("[*] =================================================================")
    logger.info(f"[*] Usando mock_box em: {PASTA_VENDAS}")
