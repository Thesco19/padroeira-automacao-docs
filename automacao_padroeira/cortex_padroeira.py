...
TOKEN_TELEGRAM = os.environ.get("TOKEN_TELEGRAM")  # ← SUBSTITUA PELO SEU TOKEN REAL
...
def comando_fechar(message):
    ...
    if not dados:
        bot.reply_to(
            message,
            "❌ Erro: Arquivo 'fechamento_caixa.txt' não encontrado na pasta do laboratório.",
        )
        return
...
def comando_ok(message):
    bot.reply_to(
        message, "⚡ *Córtex Lab:* Comparando planilhas e checando paridade..."
    )
    ...
    if not os.path.exists(diario_path):
        bot.reply_to(
            message,
            f"❌ Planilha de teste '{os.path.basename(diario_path)}' não encontrada no lab.",
        )
        return
    ...
    datas_cx2 = [
        ws_cx2.cell(row=1, column=c).value
        for c in range(2, ws_cx2.max_column + 1)
        if ws_cx2.cell(row=1, column=c).value
    ]

    wb_diario = load_workbook(diario_path, data_only=True)
    ws_diario = wb_diario.active
    datas_diario = [
        ws_diario.cell(row=1, column=c).value
        for c in range(2, ws_diario.max_column + 1)
        if ws_diario.cell(row=1, column=c).value
    ]

    str_cx2 = {d.strftime("%Y-%m-%d") for d in datas_cx2 if isinstance(d, datetime)}
    str_diario = {
        d.strftime("%Y-%m-%d") for d in datas_diario if isinstance(d, datetime)
    }
    ...
    if pendentes:
        bot.send_message(
            message.chat.id,
            f"🔄 *Datas pendentes detectadas no Caixa 2:* {len(pendentes)} dia(s). Rodando motores...",
        )
    else:
        bot.send_message(
            message.chat.id,
            "✅ Paridade total encontrada! Nenhuma data nova detectada, mas os motores serão executados para garantir a consistência.",
        )
    ...
    try:
        ...
        bot.send_message(
            message.chat.id,
            "🚀 *CONSOLIDAÇÃO DE TESTE CONCLUÍDA!* Verifique os arquivos no lab.",
        )
...
if __name__ == "__main__":
    logger.info("[*] LAB CÓRTEX: Ativo e escutando o Telegram...")
