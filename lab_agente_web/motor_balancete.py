...
# -*- coding: utf-8 -*-
"""
Engine de Consolidação - FASE 2: Transposição Diário -> Balancete
Córtex de Desenvolvimento: work_out/lab
Diretriz de Execução: Sobreposição Agressiva
"""
...
def injetar_balancete():
    diario_path = os.path.join(PASTA_RESTAURANTE_ANO, f"Movto_diario.{AAMM}.xlsx")
    pad_path = os.path.join(PASTA_RESTAURANTE_ANO, f"Pad{AAMM}.xlsx")
    
    if not os.path.exists(diario_path) or not os.path.exists(pad_path):
        print("[-] Erro Crítico: Arquivos da Fase 2 (Diário ou Pad) não encontrados no Box.")
        return

    print("[*] Iniciando Fase 2: Transposição de Matriz (Diário -> Balancete Pad)...")
    ...
    if not carga_por_dia:
        print("[-] Nenhum dia válido encontrado no Diário Mensal.")
        return

    print(f"[*] Carga extraída do Diário! Dias capturados: {sorted(list(carga_por_dia.keys()))}")
    ...
    if nome_aba in wb_pad.sheetnames:
        ...
        print(f"[-] Aviso: Aba '{nome_aba}' não encontrada. Usando a aba ativa: {ws_pad.title}")
        ...
        if not linha_destino:
            for r in range(2, 32):
                if not ws_pad.cell(row=r, column=1).value:
                    ...
                    print(f"    [+] Novo dia ({dia}) inserido na linha {linha_destino} do Balancete.")
                    break

            for letra_coluna, valor in valores.items():
                if linha_destino:
                     print(f"    -> [Sobreposição Ativa] Injetando dados do Dia {dia} na Linha {linha_destino}")
            for letra_coluna, valor in valores.items():
                # ADICIONE ESTA VERIFICAÇÃO: Pula a coluna Q se você não quer que ela seja sobrescrita
                if letra_coluna == 'Q':
                    continue
    ...
    if linhas_modificadas > 0:
        try:
            ...
            print(f"\n[+] Fase 2 Concluída! {linhas_modificadas} dias sobrepostos com sucesso no Balancete (Pad{AAMM}.xlsx).")
        except Exception as e:
            print(f"[-] Erro ao salvar o Balancete: {e}")

if __name__ == "__main__":
    print("[*] ==================================================================")
    print("[*] PHASE 2: MOTOR DE TRANSPOSIÇÃO E SOBREPOSIÇÃO AGRESSIVA")
    print("[*] ==================================================================")
    injetar_balancete()
